<template>
	<div class="monthlyRefund">
		<mt-header fixed title="日回款">
			<router-link to="/sale/personalChild/clientRefund" slot="left">
				<mt-button icon="back">返回</mt-button>
			</router-link>
		</mt-header>
		<ul class="list">
			<li v-for="(item,index) in data" :key="item.path">
				<router-link :index="index" :to="{path:'/sale/personalChild/detailRefund',query:{id:item.id}}">
					<p>{{item.name}}</p>
